...
from pathlib import Path
config_path = Path(
    os.environ.get("SAM2_CONFIG", str(REPO_ROOT / "config.yaml"))
)
...
class SAMTracker():
    def __init__(self, tracker_name="sam21-L", checkpoint=None, model_cfg=None, device=None):
        ...
        - tracker_name (str): Name of the tracker to use. Options are:
            - "sam21-L": DAM4SAM (2.1) Hiera Large
            - "sam21-B": DAM4SAM (2.1) Hiera Base+
            - "sam21-S": DAM4SAM (2.1) Hiera Small
            - "sam21-T": DAM4SAM (2.1) Hiera Tiny
            - "sam2-L": DAM4SAM (2) Hiera Large
            - "sam2-B": DAM4SAM (2) Hiera Base+
        ...
        for out in self.predictor.propagate_in_video(self.inference_state, start_frame_idx=self.frame_index, max_frame_num_to_track=0, return_all_masks=True):
            if len(out) == 3:
                ...
                if m_iou > 0.8 and obj_sizes_ratio >= 0.8 and obj_sizes_ratio <= 1.2 and n_pixels >= 1 and (self.frame_index - self.last_added > 5 or self.last_added == -1):
                    calculate_overlaps, Mask, RegionType = _load_vot_region_helpers()
