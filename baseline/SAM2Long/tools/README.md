...
The `vot_inference.py` script runs SAM2Long on box-initialized tracking datasets and writes one bbox text file per sequence. It supports:
...
The runner initializes with the first-frame ground-truth box, except for DIDI where it uses `first_frame_segm.txt` as a mask prompt, propagates through the whole sequence with SAM2Long, and saves results as `x,y,w,h` lines. For DIDI it also writes a VOT mask trajectory under `--output_dir/<sequence>/<sequence>.txt`. Add `--output_mask_dir` to also save binary propagated masks.
